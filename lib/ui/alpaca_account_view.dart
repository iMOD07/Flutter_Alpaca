...
class _AlpacaAccountViewState extends State<AlpacaAccountView> {
  ...
  Future<void> _handlePullToRefresh() async {
    ...
    // 1 Start the request outside of setState
    final f = _api.getAccount();

    // 2 Update the FutureBuilder synchronously (without async/await)
    if (mounted) {
      setState(() {
        _future = f;
      });
    ...
    // 3 Wait for the same Future outside setState
    try {
      ...
      if (!mounted) return;
      setState(() {
        // Any additional UI updates here only (synced)
        _lastRefresh = DateTime.now();
      });
    } catch (e) {
      // If there is a network/coding failure...etc.
      if (!mounted) return;
          ...
          Expanded(
            child: FutureBuilder<Map<String, dynamic>>(
              ...
              builder: (context, snap) {
                ...
                final optionsApprovedLevel = _s(data, 'options_approved_level');
                ...
                final effectiveBuyingPower = _s(data, 'effective_buying_power');
                        ...
                        child: Padding(
                          ...
                          child: Column(
                            ...
                            children: [
                              ...
                              Text('Options Buying Power: $optionsBuyingPower'),
                              ...
                              Text('Pending Reg TAF Fees: $pendingRegTafFees'),
                            ],
                          ),
                        ),
                      ),
